  ...
  const articles = [
    {
      ...
      categoryClasses:
        "bg-indigo-600/20 font-orbitron text-sm leading-relaxed text-white px-3 py-1 rounded-full",
    },
    {
      category: "Technology",
      date: "May 18, 2025",
      ...
      categoryClasses:
        "bg-pink-600/20 font-orbitron text-sm leading-relaxed text-white px-3 py-1 rounded-full",
    },
  ];
    ...
    >
      <div className="container mx-auto px-6 max-w-7xl">
        ...
        <div className="text-center mb-16">
          ...
          <p className="font-orbitron text-[1.2rem] leading-relaxed font-medium mt-3">
            Insights, stories and ideas from our team
          </p>
        </div>

        {/* Articles Grid */}
        <div className="grid md:grid-cols-2 gap-10 place-items-center items-stretch">
          {articles.map((article, idx) => (
            <article
              key={idx}
              className="w-full max-w-xl p-6 rounded-2xl border border-gray-800 bg-[#0d0d0d]
                shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 
                backdrop-blur-xl h-full"
              data-scroll
            >
              {/* Top Bar */}
              <div className="flex items-center justify-between mb-3 text-sm text-gray-400 font-orbitron">
                <span className={article.categoryClasses}>{article.category}</span>
                <span>{article.date}</span>
              </div>

              {/* Title */}
              <h3 className="text-2xl font-extrabold mb-4 text-white">{article.title}</h3>
              ...
              <div className="relative mb-5">
                ...
                />
                <div className={article.badgeClasses}>{article.badge}</div>
              </div>

              {/* Content */}
              <div className="font-orbitron text-sm leading-relaxed text-white space-y-3 text-left">
